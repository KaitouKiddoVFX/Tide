...
namespace Tide
{
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		{
			TD_CORE_ASSERT("Could not open file '{0}'", filepath);
		}
	...
	void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSource)
	{
		...
		TD_CORE_ASSERT(shaderSource.size() <= 2, "We only support 2 shaders for now");
		...
		for (auto& kv : shaderSource)
		{
			...
			if (isComplied == GL_FALSE)
			{
				...
				TD_CORE_ASSERT("{0}", infoLog.data());
				TD_CORE_ASSERT(false, "Shader compilaction failuer!");
				break;
			}
		...
		// Note the different fuctions here: glGetProgram* instead of glGetShader*
		GLint isLinked = 0;
